...
TEST(TransformationTests, ConvertStridedSliceToCropTests3) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        auto input = std::make_shared<ngraph::opset1::Parameter>(ngraph::element::f32, ngraph::Shape{ 1, 3, 11, 64 });
        auto slice_begin = ngraph::opset1::Constant::create(ngraph::element::i64, ngraph::Shape{ 4 }, { 0, -2, 1, 0 });
        auto slice_end = ngraph::opset1::Constant::create(ngraph::element::i64, ngraph::Shape{ 4 }, { 0, -1, -1, 0 });
        auto slice_stride = ngraph::opset1::Constant::create(ngraph::element::i64, ngraph::Shape{ 4 }, { 1, 1, 1, 1 });

        std::vector<int64_t> begin_mask = { 1, 0, 0, 1 };
        std::vector<int64_t> end_mask = { 1, 0, 0, 1 };
        std::vector<int64_t> new_axis_mask = { 0, 0, 0, 0 };
        std::vector<int64_t> shrink_axis_mask = { 0, 1, 0, 0 };
        std::vector<int64_t> ellipsis_mask = { 0, 0, 0, 0 };
...

    {
        ...
        std::vector<int64_t> axes = { 0, 1, 2, 3 };
        std::vector<int64_t> dim = { 1, 1, 9, 64 };
        std::vector<int64_t> offset = { 0, 1, 1, 0 };

        auto crop = std::make_shared<ngraph::op::CropIE>(input, axes, dim, offset);
        crop->set_friendly_name("strided_slice/Crop");

        auto reshape = ngraph::op::util::reshapeTo(crop, { 1, 9, 64 });
        reshape->set_friendly_name("strided_slice");
