...
TEST_F(RemoteBlob_Test, smoke_canInferOnUserContextAndQueue) {
#if defined _WIN32
    GTEST_SKIP();
#endif
    auto fn_ptr = ngraph::builder::subgraph::makeSplitMultiConvConcat();
    ...
    auto remote_context = make_shared_context(*ie, CommonTestUtils::DEVICE_GPU, ocl_instance->_context.get(), { ocl_instance->_queue.get() });
    auto exec_net_shared = ie->LoadNetwork(net, remote_context);
    auto inf_req_shared = exec_net_shared.CreateInferRequest();
    ...
    // If StartAsync just runs first pipeline stage in separate thread and returns immediately (default behavior), then following barrier and buffer copy
    // will be scheduled before inference primitives, thus output_buffer will contain something invalid
    ...
    // Note that this approach has many limitations at this moment which can lead to invalid inference result or calling thread block on StartAsync call
    inf_req_shared.Wait();
