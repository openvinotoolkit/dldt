...
std::string ConcatWithIntermediateTransformation::getTestCaseName(testing::TestParamInfo<ConcatWithIntermediateTransformationParams> obj) {
    ...
    std::tie(netPrecision, inputShapes, targetDevice, params, transparentIntermediate, multichannel, config) = obj.param;
...
InferenceEngine::Blob::Ptr ConcatWithIntermediateTransformation::GenerateInput(const InferenceEngine::InputInfo &info) const {
    ...
    bool multichannel;
    std::pair<std::string, std::map<std::string, std::string>> config;
    std::tie(netPrecision, inputShape, targetDevice, trasformationParams, transparentIntermediate, multichannel, config) = this->GetParam();
...
void ConcatWithIntermediateTransformation::SetUp() {
    ...
    bool multichannel;
    std::pair<std::string, std::map<std::string, std::string>> config;
    std::tie(ngPrecision, inputShape, targetDevice, trasformationParams, transparentIntermediate, multichannel, config) = this->GetParam();
...
void ConcatWithIntermediateTransformation::validate() {
    ...
    bool multichannel;
    std::pair<std::string, std::map<std::string, std::string>> config;
    std::tie(netPrecision, inputShape, targetDevice, params, transparentIntermediate, multichannel, config) = this->GetParam();
