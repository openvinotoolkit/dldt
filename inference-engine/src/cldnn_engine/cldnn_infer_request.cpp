        ...
        if (!is_remote) {
            if (preProcessingRequired(foundInput, data)) {
                ...
                if (data->buffer() == nullptr)
                    IE_THROW() << str_input_not_allocated << " Input name: \'" << name << "\'";
                _inputs[name] = data;
            }
        }
    } else {
...
void CLDNNInferRequest::exec_and_parse() {
        ...
        for (auto& item : _inputs) {
            ...
            if (is_nv12 || is_batched) {
                ...
                for (auto i = 0; i < expected_batch; i++) {
                    ...
                    prepare_input(y_name, nv12_ptr->y());
                    prepare_input(uv_name, nv12_ptr->uv());
                }
            } else {
                // regular blob
                prepare_input(inputName, inputBlob);
            }
        }
...
void CLDNNInferRequest::prepare_output(const cldnn::primitive_id& outputName, Blob::Ptr& outputBlob) {
    ...
    _nw_ptr->set_output_memory(internalName, outputMem, true, true, true);
}
