            ...
            for (int ow = 0; ow < ow_step; ow++) {
                for (int kh = 0; kh < jcp_.kh; kh++) {
                    for (int kw = 0; kw < jcp_.kw; kw++) {
                        ...
                        vmovd(xmm_cur_height, table_val(3));
                        vpsubd(xmm_cur_height, xmm_cur_height, xmm_ih_in);

                        vroundps(xmm_h_low, xmm_map_h, 1);
                        vcvtps2dq(xmm_h_low, xmm_h_low);
                        vpmaxsd(xmm_h_low, xmm_h_low, table_val(0));

                        if (jcp_.with_bi_pad) {
                            vmovdqu(xmm_h_high, xmm_h_low);
                            vpaddd(xmm_h_high, xmm_h_high, table_val(5));
                        } else {
                            vroundps(xmm_tmp, xmm_map_h, 2);
                            vcvtps2dq(xmm_tmp, xmm_tmp);
                            vpminsd(xmm_h_high, xmm_tmp, xmm_cur_height);
                        }

                        vmovd(xmm_cur_width, table_val(4));
                        vpsubd(xmm_cur_width, xmm_cur_width, xmm_iw_in);

                        vroundps(xmm_w_low, xmm_map_w, 1);
                        vcvtps2dq(xmm_w_low, xmm_w_low);
                        vpmaxsd(xmm_w_low, xmm_w_low, table_val(0));

                        if (jcp_.with_bi_pad) {
                            vmovdqu(xmm_w_high, xmm_w_low);
                            vpaddd(xmm_w_high, xmm_w_high, table_val(5));
                        } else {
                            vroundps(xmm_tmp, xmm_map_w, 2);
                            vcvtps2dq(xmm_tmp, xmm_tmp);
                            vpminsd(xmm_w_high, xmm_tmp, xmm_cur_width);
                        }
                        ...
                        L(ic_loop_tail);
                        {
                            ...
                            cvtdq2ps(xmm_v2, xmm_v2);
                            // w_high <= cur_width - 1
                            movq(xmm_tmp, xmm_w_high);
                            pcmpgtd(xmm_tmp, xmm_cur_width);
                            movq(reg_tmp_64, xmm_tmp);
                            cmp(reg_tmp_32, 0);
                            // je(v2_condition_end_tail, T_NEAR);
                            ...
                            movq(reg_tmp_64, xmm_tmp);
                            cmp(reg_tmp_32, 0);
                            // je(v3_condition_end_tail, T_NEAR);
                            ...
                            cvtdq2ps(xmm_v4, xmm_v4);
                            // w_high <= cur_width - 1
                            movq(xmm_tmp, xmm_w_high);
                            pcmpgtd(xmm_tmp, xmm_cur_width);
                            movq(reg_tmp_64, xmm_tmp);
                            cmp(reg_tmp_32, 0);
                            // je(v4_condition_end_tail, T_NEAR);
...
void MKLDNNDeformableConvolutionNode::initSupportedPrimitiveDescriptors() {
    ...
    size_t parentEdgesLen = getParentEdges().size();
    InferenceEngine::LayerConfig config;
    config.dynBatchSupport = false;
    config.inConfs.resize(parentEdgesLen);
