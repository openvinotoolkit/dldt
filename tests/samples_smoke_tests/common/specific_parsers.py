...
def parse_text_detection_to_array(stdout):
    arr = []
    # Sample print not only result of prediction, but also info about IE, need to not pay attention to it in testing
    skipping_position = 0 if np.any(['CTRL+C' in out for out in stdout]) else 1
    for line in stdout:
        # Parse output
        if 'CTRL+C' not in line and skipping_position == 0:
            continue
        elif 'CTRL+C' in line:
            skipping_position = 1
            continue
        elif 'CTRL+C' not in line and skipping_position == 1:
            arr += line.replace('\n', '').split(',')
...
def parse_hello_reshape_ssd(stdout):
    ...
    for line in stdout:
        if 'Resulting input shape' in line:
            if re.match("^Resulting input shape\s+=\s+\[\d+,\d+,\d+,\d+]", line) is None:
                is_ok = False
            ...
            if re.match("^Resulting output shape\s+=\s+\[\d+,\d+,\d+,\d+]", line) is None:
                is_ok = False
                log.error('Wrong output line: {}, while the test expects the following format: 4d shape'
